...
impl FromStr for AccountId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 3 {
            return Err(LwsError::CaipParseError {
                message: format!("expected 'namespace:reference:address', got '{}'", s),
            });
        }
        if parts[2].is_empty() {
            return Err(LwsError::CaipParseError {
        ...
        ChainId::validate_namespace(&chain_id.namespace)?;
        ChainId::validate_reference(&chain_id.reference)?;
        Ok(AccountId {
            chain_id,
...
mod tests {
    ...
    #[test]
    fn test_account_id_evm() {
        let acct: AccountId =
            "eip155:1:0x2c7536E3605D9C16a7a3D7b1898e529396a65c23".parse().unwrap();
        assert_eq!(acct.chain_id.namespace, "eip155");
        assert_eq!(acct.chain_id.reference, "1");
        assert_eq!(acct.address, "0x2c7536E3605D9C16a7a3D7b1898e529396a65c23");
    }

    #[test]
    fn test_account_id_solana() {
        let acct: AccountId =
            "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp:7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv"
                .parse()
                .unwrap();
        assert_eq!(acct.chain_id.namespace, "solana");
    }

    #[test]
    fn test_account_id_cosmos() {
        let acct: AccountId =
            "cosmos:cosmoshub-4:cosmos1hsk6jryyqjfhp5dhc55tc9jtckygx0eph6dd02"
                .parse()
                .unwrap();
        assert_eq!(acct.chain_id.reference, "cosmoshub-4");
        assert_eq!(
            acct.address,
    ...
    #[test]
    fn test_account_id_bitcoin() {
        let acct: AccountId =
            "bip122:000000000019d6689c085ae165831e93:bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4"
                .parse()
                .unwrap();
        assert_eq!(acct.chain_id.namespace, "bip122");
    }

    #[test]
    fn test_account_id_tron() {
        let acct: AccountId = "tron:mainnet:TJCnKsPa7y5okkXvQAidZBzqx3QyQ6sxMW"
            .parse()
            .unwrap();
        assert_eq!(acct.chain_id.namespace, "tron");
        assert_eq!(acct.address, "TJCnKsPa7y5okkXvQAidZBzqx3QyQ6sxMW");
    }

    #[test]
    fn test_account_id_reject_missing_address() {
        assert!("eip155:1:".parse::<AccountId>().is_err());
    }

    #[test]
    fn test_account_id_reject_no_address_part() {
        assert!("eip155:1".parse::<AccountId>().is_err());
    }
    ...
    fn test_account_id_display_roundtrip() {
        ...
        assert_eq!(s, "eip155:1:0x2c7536E3605D9C16a7a3D7b1898e529396a65c23");
        let acct2: AccountId = s.parse().unwrap();
