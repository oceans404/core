# Node.js SDK — `@local-wallet-standard/node`
...
```javascript
import { generateMnemonic, deriveAddress } from "@local-wallet-standard/node";
...
```javascript
import { signMessage, signTransaction, signAndSend } from "@local-wallet-standard/node";
...
const vault = mkdtempSync(join(tmpdir(), "lws-"));
